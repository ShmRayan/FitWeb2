...
const CasquettePromo = () => {
  ...
  useEffect(() => {
    ...
    const storedReviews = JSON.parse(localStorage.getItem('casquettePromoReviews')) || [
      { id: 1, text: "Great promo on a stylish cap! Love it.", author: "Alice", rating: 5, userId: "Alice" },
      { id: 2, text: "Good quality but the promo price could be better.", author: "Bob", rating: 4, userId: "Bob" },
    ];
  ...
  useEffect(() => {
    ...
    localStorage.setItem('casquettePromoReviews', JSON.stringify(reviews));
  }, [reviews]);

  const product = {
    id: 15,  // Assurez-vous que cet identifiant est unique
    name: 'Casquette Promo',
    price: '39.99$',
    image: `${process.env.PUBLIC_URL}/assets/casquette1.webp`,
    description: 'Stylish and comfortable caps for everyday wear. Special promo price!',
    advantages: [
