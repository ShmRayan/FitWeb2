...
const DebardeurPromo = () => {
  ...
  useEffect(() => {
    ...
    const storedReviews = JSON.parse(localStorage.getItem('debardeurPromoReviews')) || [
      { id: 1, text: "Fantastic promo price for a great tank top!", author: "Alice", rating: 5, userId: "Alice" },
      { id: 2, text: "Good quality, but it runs a bit small.", author: "Bob", rating: 4, userId: "Bob" },
    ];
  ...
  useEffect(() => {
    ...
    localStorage.setItem('debardeurPromoReviews', JSON.stringify(reviews));
  }, [reviews]);

  const product = {
    id: 16,
    name: 'Debardeur Promo',
    price: '39.99$',
    image: `${process.env.PUBLIC_URL}/assets/debardeur1.jpeg`,
    description: 'Comfortable tank tops for your workouts and casual wear. Special promo price!',
    advantages: [
      'Breathable fabric',
      'Stylish design',
